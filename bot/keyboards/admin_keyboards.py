...
# ИСПРАВЛЕНИЕ: Переход на использование фабрик CallbackData для надежности.
# =================================================================================
...
def get_stats_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    """
    builder = InlineKeyboardBuilder()
    builder.button(text="👤 Общая (Пользователи)", callback_data=AdminCallback(action="stats", value="general").pack())
    builder.button(text="💎 Игровая (Майнинг)", callback_data=AdminCallback(action="stats", value="mining").pack())
    builder.button(text="📈 Топ действий", callback_data=AdminCallback(action="stats", value="commands").pack())
...
def get_system_actions_keyboard() -> InlineKeyboardMarkup:
    ...
    """
    builder = InlineKeyboardBuilder()
    builder.button(text="🗑 Очистить кэш ASIC", callback_data=AdminCallback(action="system", value="clear_asic_cache").pack())
