...
def get_market_listings_keyboard(
    listings: list[MarketListing], current_page: int, total_pages: int
    ...
    if total_pages > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text=f"{current_page + 1}/{total_pages}", callback_data="do_nothing"
            )
    ...
    builder.row(
        InlineKeyboardButton(
            text="Продать своё оборудование",
            callback_data=MarketCallback(action="sell_start").pack(),
        )
    )
...
def get_choose_asic_to_sell_keyboard(
    user_asics: list[AsicMiner],
) -> InlineKeyboardMarkup:
    """
    ...
    for asic in user_asics:
        builder.row(
            InlineKeyboardButton(
                ...
                callback_data=MarketCallback(
                    action="sell_select", asic_id=asic.id
                ).pack(),
            )
        )
...
def get_confirm_buy_keyboard(listing_id: str) -> InlineKeyboardMarkup:
    ...
    builder.button(
        ...
        callback_data=MarketCallback(
            action="buy_confirm", listing_id=listing_id
        ).pack(),
    )
    builder.button(
        text="❌ Отмена", callback_data=MarketCallback(action="buy_cancel").pack()
    )
    builder.adjust(2)
    return builder.as_markup()
