...
def get_mining_menu_keyboard(is_session_active: bool) -> InlineKeyboardMarkup:
    ...
    if not is_session_active:
        builder.button(
            text="▶️ Начать сессию", callback_data=GameCallback(action="shop").pack()
        )

    builder.button(
        text="🏠 Моя ферма", callback_data=GameCallback(action="my_farm").pack()
    )
    builder.button(
        text="💡 Электричество", callback_data=GameCallback(action="electricity").pack()
    )
    builder.button(
        text="🤝 Пригласить друга", callback_data=GameCallback(action="invite").pack()
    )
    builder.button(
        text="⬅️ Назад в меню", callback_data=MenuCallback(level=0, action="main").pack()
    )
...
def get_shop_keyboard(asics: list[AsicMiner], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for asic in asics[start_offset:end_offset]:
        ...
        profit_str = (
            f" ({_fmt_money(asic.profitability, 2)}$/день)"
            if getattr(asic, "profitability", None) is not None
            else ""
        )
        builder.button(
    ...
    if total_pages > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text=f"{page + 1}/{total_pages}", callback_data="do_nothing"
            )
        )
    ...
    builder.row(
        InlineKeyboardButton(
            text="⬅️ Назад в меню", callback_data=GameCallback(action="main_menu").pack()
        )
    )
    builder.adjust(1)
    return builder.as_markup()
...
def get_confirm_purchase_keyboard(item_id: str) -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="✅ Купить",
        callback_data=GameCallback(action="confirm_purchase", value=item_id).pack(),
    )
    builder.button(
        text="❌ Отмена", callback_data=GameCallback(action="main_menu").pack()
    )
    builder.adjust(2)
    return builder.as_markup()
...
def get_my_farm_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(
        text="💰 Вывести средства", callback_data=GameCallback(action="withdraw").pack()
    )
    builder.button(
        text="⬅️ Назад в меню", callback_data=GameCallback(action="main_menu").pack()
    )
...
def get_withdraw_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(
        text="✅ Понятно", callback_data=GameCallback(action="main_menu").pack()
    )
    return builder.as_markup()
            ...
            if is_current:
                builder.button(
                    ...
                    callback_data=GameCallback(
                        action="tariff_select", value=name
                    ).pack(),
                )
            else:
                builder.button(
...
def get_currency_selection_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="USD ($)",
        callback_data=CalculatorCallback(action="currency", value="usd").pack(),
    )
    builder.button(
        text="RUB (₽)",
        callback_data=CalculatorCallback(action="currency", value="rub").pack(),
    )
...
def get_asic_selection_keyboard(
    asics: list[AsicMiner], page: int
) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    start_offset = page * PAGE_SIZE
    end_offset = start_offset + PAGE_SIZE
    ...
    for i, asic in enumerate(asics[start_offset:end_offset], start=start_offset):
        builder.button(
            text=asic.name,
            callback_data=CalculatorCallback(action="select_asic", asic_index=i).pack(),
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=CalculatorCallback(action="page", page=page - 1).pack(),
            )
        )
    if end_offset < len(asics):
        nav_buttons.append(
...
def get_calculator_result_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="⬅️ Новый расчёт",
        callback_data=MenuCallback(level=1, action="calculator").pack(),
    )
    builder.button(
        text="🏠 Главное меню",
