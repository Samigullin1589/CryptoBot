# =================================================================================
# Файл: bot/config/settings.py (ФИНАЛЬНАЯ ВЕРСИЯ - УЛУЧШЕННАЯ И СТАБИЛЬНАЯ)
# Описание: Единая, строго типизированная система конфигурации.
# УЛУЧШЕНИЕ: Повышена строгость типизации для поля admin_ids для обеспечения
#           безупречной работы и предсказуемости.
# =================================================================================
...
class Settings(BaseSettings):
    ...
    # Тип изменен с `Any` на `List[int]`. Это более строгая и корректная декларация,
    # которая отражает тип данных ПОСЛЕ работы валидатора.
    # Pydantic сначала выполнит валидатор `parse_admin_ids` (из-за `mode='before'`),
    # а затем проверит, что результат соответствует `List[int]`.
    # `alias` по-прежнему указывает Pydantic на переменную окружения `ADMIN_USER_IDS`.
    ...
    def parse_admin_ids(cls, v: Any) -> List[int]:
        """
        Надежный валидатор, который преобразует строку '123,456' 
        из переменных окружения в список целых чисел [123, 456].
        Корректно обрабатывает пустые строки и уже существующие списки.
        """
