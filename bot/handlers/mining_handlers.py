...
async def handle_start_mining(call: CallbackQuery, redis_client: redis.Redis, scheduler: AsyncIOScheduler, asic_service: AsicService):
    ...
    job = scheduler.add_job('bot.services.mining_tasks:end_mining_session', 'date', run_date=run_date, args=[user_id], id=f"mining_job_{user_id}", replace_existing=True)
    session_data = { "start_time": int(time.time()), "job_id": job.id, "asic_name": selected_asic.name, "asic_profitability_per_day": selected_asic.profitability, "asic_power": selected_asic.power or 0 }
    await redis_client.hset(f"mining:session:{user_id}", mapping=session_data)
    await call.message.edit_text(f"✅ Вы успешно запустили майнинг на <b>{selected_asic.name}</b>!\n\nСессия продлится {settings.MINING_DURATION_SECONDS / 3600:.0f} часов. Уведомление о завершении придет в этот чат.", reply_markup=get_mining_menu_keyboard())
...
async def handle_withdraw(call: CallbackQuery, redis_client: redis.Redis):
    ...
    # Проверяем, нет ли активной сессии, чтобы избежать путаницы
    if await redis_client.exists(f"mining:session:{user_id}"):
    ...
    if balance < 1.0: # Устанавливаем минимальный порог для вывода
        await call.answer("ℹ️ Ваш баланс слишком мал для вывода. Накопите хотя бы 1 монету.", show_alert=True)
        return
        
    # Формула расчета скидки: 1% + 1% за каждые 50 монет, но не более 10%
