...
def _kb_top(symbols: list[str], quote: str) -> InlineKeyboardMarkup:
    ...
    rows.append(
        [
            InlineKeyboardButton(
                text=f"🔄 Обновить {quote}", callback_data=f"price:refresh:{quote}"
            )
        ]
    )
...
async def cmd_price(message: Message, deps) -> None:
    ...
    default_quote = getattr(
        getattr(deps.settings, "price_service", object()), "default_quote", "USDT"
    )
    ...
    if price is None:
        await message.answer(
            f"❌ Не удалось получить цену {symbol}/{quote}. Попробуйте позже."
        )
        return

    text = f"<b>{symbol}/{quote}</b>: <code>{_fmt_price(price)}</code>"
    top = await _try_call(getattr(deps, "price_service", None), "_get_top_symbols") or [
        "BTC",
        "ETH",
        "BNB",
        "SOL",
    ...
    await message.answer(
        text, parse_mode="HTML", reply_markup=_kb_top(list(top), quote)
    )
...
async def cb_price(call: CallbackQuery, deps) -> None:
    ...
    if parts[1] == "refresh":
        ...
        top = await _try_call(
            getattr(deps, "price_service", None), "_get_top_symbols"
        ) or ["BTC", "ETH", "BNB", "SOL", "XRP"]
    ...
    await call.message.edit_text(
        text, parse_mode="HTML", reply_markup=call.message.reply_markup
