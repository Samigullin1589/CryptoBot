...
async def handle_check_command(message: Message, deps: Deps):
    ...
    if not target_user:
        ...
        target_user = await deps.user_service.get_user(message.from_user.id)

    if not target_user:
        # Эта ситуация маловероятна, но возможна, если пользователь еще не в базе
    ...
    # Отключаем превью ссылок, чтобы username не разворачивался в большую карточку
