...
Кратко: Удалены посторонние символы в начале файла, добавлен унифицированный вызов AI (поддержка обеих сигнатур get\_structured\_response), усилены логирование и эвристики. Совместимо с Render, публичные интерфейсы сохранены.
    ...
    return {
        ...
        "properties": {
            ...
            "is_potential_phishing": {
                "type": "BOOLEAN",
                "description": "True если присутствуют признаки фишинга."
            }
        },
...
async def _call_structured(
    ...
    try:
        ...
        logger.info("Fallback на старую сигнатуру get_structured_response: %s", e)
        try:
            ...
            logger.error("Старая сигнатура get_structured_response упала: %s", e2, exc_info=True)
            return None
