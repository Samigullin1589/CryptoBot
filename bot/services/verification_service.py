...
class VerificationService:
    ...
    async def get_user_for_verification(self, user_id: int) -> User | None:
    ...
    async def set_verification_status(
        ...
        logger.info(
            f"Статус верификации для пользователя {user_id} изменен на {is_verified}."
        )
        return True
    ...
    async def update_deposit(self, user_id: int, amount: float) -> bool:
        ...
        if amount < 0:
            logger.warning(
                f"Попытка установить отрицательный депозит ({amount}) для пользователя {user_id}."
            )
            return False
