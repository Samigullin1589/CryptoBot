...
class MiningService:
    ...
    @staticmethod
    def _parse_hashrate_to_ths(hashrate_str: str) -> float | None:
        """
    ...
    async def calculate_btc_profitability(
        ...
        if any(isinstance(res, Exception) or res is None for res in required_data):
            logger.error(
                f"Failed to fetch market data for calculation. Results: {results}"
            )
            return None
        ...
        logger.info(
            f"Calculation successful. Net profit: ${net_profit_usd_daily:.2f}/day."
        )
        return result_model
