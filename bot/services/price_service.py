...
class PriceService:
    def __init__(self, coin_list_service: CoinListService):
        ...
        Зависит от CoinListService для получения информации об алгоритмах.
        """
        self.coin_list_service = coin_list_service
        # Кэш для цен, является атрибутом экземпляра
        self.cache = TTLCache(maxsize=100, ttl=300)

    # Используем lambda, чтобы декоратор @cached мог получить доступ к self.cache
    # Также используем repr(args) для создания ключа кэша
    @cached(cache=lambda self: self.cache, key=lambda self, query: repr([query]))
    async def get_crypto_price(self, query: str) -> Optional[CryptoCoin]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Добавляем паузу при ошибке 429
            ...
            if cg_search_data and cg_search_data.get('coins'):
                ...
                if market_data_list:
                    ...
                    # Используем другой сервис для получения списка монет
            ...
            if cp_search_data and cp_search_data.get('currencies'):
                # Ищем наиболее релевантную монету
