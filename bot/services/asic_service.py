...
class AsicService:
    ...
    def __init__(
        ...
        config: AsicServiceConfig,
    ):
        self.redis = redis
        self.parser_service = parser_service
        self.config = config
        self.keys = KeyFactory
        self._specs_cache: dict[str, dict[str, Any]] | None = None
        self._specs_cache_time: datetime | None = None

    async def _load_fallback_asics(self) -> list[AsicMiner]:
        """Загружает список ASIC из локального резервного JSON-файла."""
        try:
            with open(self.config.fallback_file_path, encoding="utf-8") as f:
                data = json.load(f)
            asics = [AsicMiner.model_validate(item) for item in data]
            logger.warning(
                f"Загружено {len(asics)} ASIC из резервного файла: {self.config.fallback_file_path}"
            )
            return asics
        except Exception as e:
            logger.error(
                f"Критическая ошибка: не удалось загрузить резервный файл ASIC: {e}",
                exc_info=True,
            )
            return []
    ...
    def _calculate_net_profit(
        gross_profit: float, power_watts: int, electricity_cost: float
    ...
    async def _get_hardware_specs_from_cache(self) -> dict[str, dict[str, Any]] | None:
        """Получает и кэширует справочник спецификаций оборудования."""
        cache_lifetime = 3600 * 12  # 12 часов
        if (
            self._specs_cache
            and self._specs_cache_time
            and (datetime.now(timezone.utc) - self._specs_cache_time).total_seconds()
            < cache_lifetime
        ):
    ...
    def _intelligent_merge(
        self, sources: list[list[AsicMiner]]
    ) -> dict[str, AsicMiner]:
        ...
        for source_list in sources:
            ...
            for asic_to_merge in source_list:
                ...
                if best_match:
                    ...
                    if (
                        not existing_asic.power or existing_asic.power == 0
                    ) and asic_to_merge.power:
    ...
    async def update_asic_list_from_sources(self) -> int:
        ...
        if not valid_sources:
            logger.error(
                "Все онлайн-источники данных ASIC недоступны. Активация резервного механизма."
            )
        ...
        if not final_asic_list:
            logger.critical(
                "Не удалось получить данные ASIC ни из онлайн-источников, ни из резервного файла. Обновление отменено."
            )
            return 0
        ...
        async with self.redis.pipeline(transaction=True) as pipe:
            ...
            pipe.set(
                self.keys.asics_last_update(), datetime.now(timezone.utc).isoformat()
            )
            await pipe.execute()
    ...
    async def get_top_asics(
        self, electricity_cost: float, count: int = 50
    ) -> tuple[list[AsicMiner], datetime | None]:
        ...
        if not await self.redis.exists(self.keys.asics_sorted_set()):
            ...
            try:
                async with RedisLock(
                    self.redis, lock_key, timeout=300, wait_timeout=60
                ):
                    if not await self.redis.exists(self.keys.asics_sorted_set()):
                        logger.info(
                            "Кэш ASIC отсутствует. Запуск принудительного обновления под блокировкой."
                        )
                        await self.update_asic_list_from_sources()
            except LockAcquisitionError:
                logger.warning(
                    "Не удалось получить блокировку для обновления ASIC (Timeout). Ожидаем завершения текущего обновления."
                )
                await asyncio.sleep(5)

        asic_keys = await self.redis.zrevrange(
            self.keys.asics_sorted_set(), 0, count - 1
        )
        if not asic_keys:
            return [], None
        ...
        last_update = (
            datetime.fromisoformat(last_update_iso) if last_update_iso else None
        )

        return asics_with_profit, last_update

    async def find_asic_by_normalized_name(
        self, normalized_name: str, electricity_cost: float
    ) -> AsicMiner | None:
