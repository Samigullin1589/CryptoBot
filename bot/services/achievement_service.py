...
class AchievementService:
    ...
    def __init__(self, market_data_service: MarketDataService, redis_client: Redis):
        """
    ...
    async def _unlock_achievement(
        ...
        """
        Атомарно выдает достижение пользователю.
        """
        context = context or {}
    ...
    async def check_market_events(self, user_id: int) -> List[Achievement]:
        """
        Проверяет рыночные события и выдает динамические достижения.
        """
        unlocked_list = []
        top_coins = await self.market_data_service.get_top_n_coins(limit=30)
        if not top_coins:
        ...
        for coin in top_coins:
            ...
            current_price, ath, price_change_24h = coin.get("current_price"), coin.get("ath"), coin.get("price_change_percentage_24h")
    ...
    async def process_static_event(
        ...
        """
        Обрабатывает статические события.
        """
