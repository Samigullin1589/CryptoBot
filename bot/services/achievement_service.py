...
class AchievementService:
    ...
    def __init__(
        self,
        redis: redis.Redis,
        config: AchievementServiceConfig,
    ...
    def _load_achievements_from_config(self, config_path: Path):
        ...
        try:
            ...
            logger.info(
                f"Загружено {len(self.static_achievements)} статических и {len(self.dynamic_achievements)} динамических достижений."
            )
        except FileNotFoundError:
            logger.error(f"Файл конфигурации достижений не найден: {config_path}")
        except Exception as e:
            logger.error(
                f"Критическая ошибка при загрузке достижений: {e}", exc_info=True
            )

    async def get_all_achievements(self) -> list[Achievement]:
        """Возвращает полный список всех достижений."""
        return list(self.static_achievements.values()) + list(
            self.dynamic_achievements.values()
        )
    ...
    async def _unlock_achievement(
        self, user_id: int, achievement: Achievement, context: dict = None
    ) -> Achievement | None:
        ...
        async with self.redis.pipeline(transaction=True) as pipe:
            pipe.hset(
                self.keys.user_achievements(user_id),
                instance_id,
                json.dumps(achievement_data_to_store, ensure_ascii=False),
            )
            pipe.hincrbyfloat(
                self.keys.user_game_profile(user_id),
                "balance",
                achievement.reward_coins,
            )
            await pipe.execute()
        logger.info(
            f"Пользователь {user_id} разблокировал достижение '{achievement.name}' ({formatted_description})"
        )
    ...
    async def check_market_events(self, user_id: int) -> list[Achievement]:
        ...
        for i, coin in enumerate(top_coins[:30]):
            ...
            if (
                coin.get("ath")
                and coin.get("current_price")
                and coin.get("current_price") >= coin.get("ath")
            ):
                if ach := self.dynamic_achievements.get("dynamic_witness_ath"):
                    if unlocked_ach := await self._unlock_achievement(
                        user_id,
                        ach,
    ...
    async def process_static_event(
        self, user_id: int, event_name: str, event_data: dict[str, Any] = None
    ) -> Achievement | None:
            ...
            if ach_id not in unlocked_ids and achievement.trigger_event == event_name:
                ...
                if achievement.trigger_conditions:
                    for key, required_value in achievement.trigger_conditions.items():
                        if (
                            key == f"{event_name}_count"
                            and current_count < required_value
                        ):
