...
class UserService:
    ...
    async def get_user(self, user_id: int) -> User | None:
        ...
        try:
            ...
            if "electricity_cost" in user_data_dict:
                user_data_dict["electricity_cost"] = float(
                    user_data_dict["electricity_cost"]
                )
            if "verification_data" in user_data_dict and isinstance(
                user_data_dict["verification_data"], str
            ):
                user_data_dict["verification_data"] = json.loads(
                    user_data_dict["verification_data"]
                )
    ...
    async def save_user(self, user: User, old_username: str | None = None) -> None:
        """
    ...
    async def get_or_create_user(self, tg_user: TelegramUser) -> tuple[User, bool]:
        ...
        if existing_user:
            ...
            if (
                ...
                or existing_user.first_name != tg_user.full_name
            ):
                existing_user.username = tg_user.username
            ...
            if update_needed:
                logger.info(
                    f"Обновлены данные для пользователя {tg_user.id} (@{tg_user.username})."
                )
        ...
        logger.info(
            f"Зарегистрирован новый пользователь {tg_user.id} (@{tg_user.username})."
        )
    ...
    async def update_user_activity(self, user_id: int):
        ...
        day_key, week_key = (
            f"users:active:day:{today_str}",
            f"users:active:week:{week_str}",
        )
    ...
    async def get_conversation_history(self, user_id: int, chat_id: int) -> list[dict]:
        ...
        raw_history = await self.redis.lrange(
            history_key, 0, self.history_max_size * 2 - 1
        )
        return [json.loads(msg) for msg in reversed(raw_history)]

    async def add_to_conversation_history(
        self, user_id: int, chat_id: int, user_text: str, ai_answer: str
    ):
        history_key = self.keys.conversation_history(user_id, chat_id)
    ...
    async def set_user_electricity_cost(self, user_id: int, cost: float) -> None:
        user, _ = await self.get_or_create_user(
            TelegramUser(id=user_id, is_bot=False, first_name="Unknown")
        )
        if user:
