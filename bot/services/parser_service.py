...
class ParserService:
    ...
    def __init__(self, http_client: HttpClient):
        ...
        # ✅ ИСПРАВЛЕНО: settings.ENDPOINTS → settings.endpoints
    ...
    async def fetch_from_whattomine(self) -> List[AsicMiner]:
        ...
        # ✅ ИСПРАВЛЕНО: self.config.WHATTOOMINE_API → self.config.whattomine_api
        if not self.config.whattomine_api: 
            return []
        
        logger.info("Парсер: Запрашиваю данные с WhatToMine API...")
        try:
            data = await self._fetch(self.config.whattomine_api)
    ...
    async def fetch_from_asicminervalue(self) -> List[AsicMiner]:
        ...
        # ✅ ИСПРАВЛЕНО: self.config.ASICMINERVALUE_URL → self.config.asicminervalue_url
    ...
    async def fetch_minerstat_hardware_specs(self) -> Optional[Dict[str, Dict[str, Any]]]:
        ...
        # ✅ ИСПРАВЛЕНО: self.config.MINERSTAT_API → self.config.minerstat_api
