...
class ParserService:
    ...
    async def _fetch(self, url: str, response_type: str = "json") -> Any | None:
        ...
        async with self.session.get(
            url, headers=headers, timeout=20, ssl=False
        ) as response:
    ...
    async def fetch_from_whattomine(self) -> list[AsicMiner]:
        ...
        try:
            ...
            for asic_id, details in data["asics"].items():
                details["vendor"] = self._extract_vendor_from_name(
                    details.get("name", "")
                )
            ...
            logger.error(
                f"Парсер: Критическая ошибка при работе с WhatToMine: {e}",
                exc_info=True,
            )
            return []
    ...
    async def fetch_from_asicminervalue(self) -> list[AsicMiner]:
        ...
        try:
            html_content = await self._fetch(
                str(self.config.asicminervalue_url), response_type="text"
            )
            if not html_content:
                return []
            ...
            if not table:
                logger.warning(
                    "Парсер: Не найдена таблица с классом 'table-hover' в HTML от AsicMinerValue."
                )
                return []
            ...
            logger.error(
                f"Парсер: Критическая ошибка при работе с AsicMinerValue: {e}",
                exc_info=True,
            )
            return []

    async def fetch_minerstat_hardware_specs(self) -> dict[str, dict[str, Any]] | None:
        ...
        try:
            ...
            return {
                ...
                if "name" in hw and hw.get("type") == "ASIC"
            }
        except Exception as e:
            logger.error(
                f"Парсер: Критическая ошибка при работе с MinerStat hardware API: {e}",
                exc_info=True,
            )
            return None
