...
class CoinAliasService:
    ...
    async def _load_aliases(self) -> dict[str, str]:
        ...
        logger.warning(
            f"Кэш псевдонимов не найден. Загрузка из файла {self._FALLBACK_FILE_PATH}"
        )
        alias_map = {}
        try:
            with open(self._FALLBACK_FILE_PATH, encoding="utf-8") as f:
                data = json.load(f)
            ...
            await self.redis.set(
                self._ALIAS_MAP_CACHE_KEY, json.dumps(alias_map), ex=3600 * 24
            )  # Кэш на 24 часа
            self._cache = alias_map
            logger.info(
                f"Успешно загружено и закэшировано {len(alias_map)} псевдонимов."
            )
            return alias_map
