...
class CryptoCenterService:
    ...
    async def _get_user_interest_profile(self, user_id: int) -> dict[str, list[str]]:
        ...
        return {
            "tags": [t for t in tags_raw],
            "interacted_coins": [c for c in coins_raw],
        }

    async def update_user_interest(
        self, user_id: int, tags: list[str] = None, coins: list[str] = None
    ):
        profile_key = self.keys.user_interest_profile(user_id)
    ...
    async def _generate_alpha(
        self, user_id: int, alpha_type: str, json_schema: dict
    ) -> list[dict[str, Any]]:
        ...
        logger.info(
            f"Generating fresh personalized {alpha_type} alpha for user {user_id} via AI Search..."
        )
        ...
        if result:
            ...
            if isinstance(result, str):
                try:
                    ...
                    logger.error(
                        f"AI returned a non-JSON string for {alpha_type}: {result}"
                    )
                    return []
            ...
            if isinstance(processed_result, list):
                logger.info(
                    f"AI Search for user {user_id} found {len(processed_result)} {alpha_type} opportunities."
                )
                await self.redis.set(
                    cache_key,
                    json.dumps(processed_result, ensure_ascii=False),
                    ex=self.config.alpha_cache_ttl_seconds,
                )
                return processed_result

        logger.warning(
            f"AI Search for user {user_id} returned no valid {alpha_type} opportunities."
        )
        return []
    ...
    async def get_airdrop_alpha(self, user_id: int) -> list[AirdropProject]:
        ...
        if not isinstance(projects_data, list):
            logger.error(
                f"Airdrop alpha data is not a list after generation: {type(projects_data)}"
            )
            return []
    ...
    async def get_live_feed_with_summary(self) -> list[NewsArticle]:
        ...
        for article in articles[:5]:
            clean_text = BeautifulSoup(article.body, "html.parser").get_text(
        ...
        for article, summary in zip(articles, summaries):
            if isinstance(summary, str):
                ...
                logger.error(
                    f"Ошибка при суммаризации статьи '{article.title}': {summary}"
                )
    ...
    async def toggle_task_status(self, user_id: int, airdrop_id: str, task_index: int):
        ...
        if await self.redis.srem(progress_key, task_index_str):
            logger.info(
                f"User {user_id} marked task {task_index} of airdrop '{airdrop_id}' as NOT completed."
            )
        else:
            await self.redis.sadd(progress_key, task_index_str)
            logger.info(
                f"User {user_id} marked task {task_index} of airdrop '{airdrop_id}' as completed."
            )
